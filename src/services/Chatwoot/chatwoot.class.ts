...
export class ChatwootClass {
  ...
  constructor(
    ...
    if (!_config.account) {
      throw new Error("ACCOUNT_ERROR: Account configuration is required");
    }

    if (!_config.token) {
      throw new Error("TOKEN_ERROR: Token configuration is required");
    }

    if (!_config.endpoint) {
      throw new Error("ENDPOINT_ERROR: Endpoint configuration is required");
    }

    this.config = _config as Required<typeof _config>;
  }
